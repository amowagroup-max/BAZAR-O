...
const HorizontalProductStrip = ({ title, products, className = "" }) => {
  ...
  return (
    <div className={`bg-[#FBE60E] rounded-lg shadow-sm p-4 ${className} w-full relative`}>
      ...
      {/* خلفية بيضاء داخل الإطار الأصفر */}
        ...
        <div className="relative overflow-hidden">
          <div className="flex space-x-4 pb-4 animate-scroll ml-[-2rem]">
            ...
            {duplicatedProducts.map((product, index) => (
              <Link
                key={index}
                to={`/product/${product.id}`}   // ✅ الانتقال لصفحة المنتج
                className={`flex-shrink-0 w-48 bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-200 block ${
                  index === 0 ? "ml-[-1rem]" : ""
                }`}
              >
